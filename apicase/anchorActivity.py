...
try:
    data = open(setting.TEST_DATA_YAML + '/api/anchor_data.yaml', encoding='utf-8')
...
@ddt.ddt
class BusinessActivityTest(unittest.TestCase):
    ...
    def test_business_registration(self, data_yaml):
        ...
        # 查询筹备中的机构/主播活动
        url = _url + '/kunheng-business/activity/all?actStatus=0&current=1&size=100'
        headers = {"Authorization": "Basic d2ViOndlYl9rdW5oZW5n",
        ...
        if len(ids) > 0:
            ...
            for i, val in enumerate(ids):
                # 地址
                url = _url + '/kunheng-business/activity/pause?id={}'.format(val)
                res = requests.get(url=url, headers=headers, verify=False)
                print(res.text)
        else:
            print("结束...")
